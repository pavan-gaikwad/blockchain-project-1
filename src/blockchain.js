...
class Blockchain {
           ...
           if(currentHeight == -1){
            ...
            //    block.hash = SHA256(block.height+block.body+block.time+block.previousBlockHash).toString();
               ...
               if(log.length > 0){
                ...
                if(this.chain[this.chain.length - 1 ].height != currentHeight){
                    reject("Height of the chain and the last block do not match");
                }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(timeElapsed > 300){
                reject("More than 5 mins elapsed")
                
            }else {
                if(! bitcoinMessage.verify(message, address, signature)){
                    reject("Message verification failed.")
                }else {
                this._addBlock(new BlockClass.Block({address, message, signature, star})).then(ok => {
                    console.log("Block added.");
                    resolve(ok);
                }).catch(err => {
                    console.log("Failure while adding the block");
                    reject("Failure while adding the block");
                    });
                }
            }
    ...
    getBlockByHeight(height) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.filter(p => p.height === parseInt(height))[0];
            if(block){
                resolve(block);
            } else {
                resolve(null);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            this.chain.forEach(block => {
                ...
                if(JSON.parse(hex2ascii(block.body)).address == address) {
                    stars.push(JSON.parse(hex2ascii(block.body)));
                }
            })
            // let ownedStars = this.chain.filter(block => JSON.parse(hex2ascii(block.data)).address == address );
